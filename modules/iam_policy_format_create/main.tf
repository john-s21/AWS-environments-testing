...
resource "aws_iam_policy" "aws_iam_policy_rendered" {
  name  = "${local.project_prefix}-${local.environment}-${element(split(".", basename(var.policy_file_path)),0)}" # TODO - nasty formatting; put into locals...or something
